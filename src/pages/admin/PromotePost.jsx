...
export default function PromotePost() {
  ...
  if (loading) return <div className="text-center py-8 text-gray-300">Loading jobs...</div>;

  return (
    <div>
      <h2 className="text-2xl font-bold mb-6 text-white">Promote Posts</h2>
      <div className="grid gap-4">
        {jobs.map((job) => (
          <div key={job._id} className="bg-black p-6 rounded-lg shadow-lg flex justify-between items-center border border-gray-700">
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-semibold text-white">{job.title}</h3>
                {job.isPromoted && <Star className="text-yellow-400 fill-yellow-400" size={18} />}
              </div>
              <p className="text-sm text-gray-400">{job.company?.name || "Admin Post"}</p>
            </div>
            {job.isPromoted ? (
              <span className="flex items-center gap-1 text-green-400"><Check size={18} />Promoted</span>
            ) : (
              <button
        ...
        {jobs.length === 0 && <p className="text-gray-400 text-center py-8">No jobs available</p>}
      </div>
    </div>
  );
}
